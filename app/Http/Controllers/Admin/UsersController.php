...
class UsersController extends Controller
{
    ...
    public function store(Request $request)
    {
      ...
      $this->validate($request,[
            ...
            'password' => 'required|min:8|max:16|regex:/^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[#?!@$%^&*-]).{6,}$/',//regex:/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*(_|[^\w])).+$/
            //regex:/^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[#?!@$%^&*-]).{6,}$/
    ...
    public function update(Request $request, $id)
    {
      /*  $user->update($request->all());
        ...
        {
            $filepath_file = 'public/userpdffile/'.$user->pdf_file;
        ...
        {
            $filepath_excel = 'public/userexcelfile/'.$user->excel_file;
            if(File::exists($filepath_excel))
            {
                File::delete($filepath_excel);
            }
