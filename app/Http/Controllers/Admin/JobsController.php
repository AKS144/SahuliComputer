...
class JobsController extends Controller
{
    ...
    {
         //Job::where('end_date','<',Carbon::now())->delete();//run beelow code
        // if(DB::table('jobs') != Null)
        // {
           ...
           if(DB::table('jobs') != Null)
            ...
            $filepath_file = 'public/jobfile/'.$job->job_file;
            if(File::exists($filepath_file))
            {
                File::delete($filepath_file);
            }
        
            $filepath_soc = 'public/jobsocial/'.$job->job_social;
            if(File::exists($filepath_soc))
            {
                File::delete($filepath_soc);
            }
    ...
    public function store(Request $request)
    {
        ...
        $this->validate($request,[           
            ...
            'job_image' => 'image|max:2048|mimes:jpeg,png,jpg,svg',
            'job_file' => 'mimes:pdf|max:2048',
            'start_date' => 'required|date',
            'end_date' => 'required|date|after:start_date',            
            'job_social' => 'image|max:2048|mimes:jpeg,png,jpg,svg',
            'job_link'  => 'max:255',
        ]);
